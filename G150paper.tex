...
\begin{figure}[!ht]
	\begin{centering}
		...
		\caption{Background subtracted residual TS map above 1 GeV with 0.1$^\circ$x 0.1$^\circ$ pixels for fixed index $\Gamma$ = 2, centered on SNR \Gone. The orange circle and translucent shading show the fit disk radius and 1$\sigma$ errors, respectively, for the extended source, the orange cross shows the position of 3FGL J0426.7+5437 (included in the background model), blue dashed circle is the extent of the radio SNR, and white dashed circle depicts \ghard{}. Bottom includes \Gone{} in the background model, top does not.
...
\begin{figure}[!ht]
	\begin{centering}
		...
		\caption{Not sure I want to include these, replot, get rid of titles, and make them look nicer if I do want to include. Top plot shows that the TS peaks at the best fit extension. Lower plot gives a sense of how significant the extension is (vs a point source) across the analyzed energy range. .
			\label{fig:1GeV_ext}}
	\end{centering}
\end{figure}
...
\begin{figure}[!ht]
	\begin{centering}
		\includegraphics[width=\columnwidth]{{G150_extProf_1GeV}.png}
		\includegraphics[width=\columnwidth]{{TSextVsEn_G150_1GeV_1TeV_sigma1_4}.png}
		\caption{Not sure I want to include these, replot, get rid of titles,  if I do want to include.
			\label{fig:1GeV_ext}}
	\end{centering}
\end{figure}
...
After determining the best fit morphology with \ptlike{} for the GeV emission coincident with SNR \Gone{}, we used those results as a starting point for our \gtlike{} maximum likelihood fit of the region to estimate the best spectral parameters for our model. The LAT data is well described by a power law across the entire energy range with a photon index, ${\rm \Gamma = 1.82 \pm 0.04}$, and energy flux above 1 GeV of ${\rm (7.17 \pm 0.73~ x 10^{-11})~ erg~cm^{-2}~ s^{-1}}$and TS = 373 \jamie{pointlike results were index = 1.80 flux = ${\rm (7.17 \pm 0.73~ x 10^{-11})~ erg~cm^{-2}~ s^{-1}}$}. We tested the \gam ~spectrum of the extended disk for spectral curvature using a log-normal model (Log Parabola), and find no significant deviation from a power law (${\rm \Delta TS \sim 1}$). \jamie{what else to include here? Systematics. Bracketing IRFs, alt iem, try varying the extension? still to be done.} Figure \ref{fig:G150SED} shows the best fit power law spectral energy distribution for the GeV emission whose morphology was described in Section $\ref{sec:LATmorph}$. What to say about the J0426
...
\begin{figure}[!ht]
	\begin{centering}
		...
		\caption{Spectral energy distribution for the extended source coincident with SNR \Gone. \jamie{Butterfly?}
			\label{fig:G150SED}}
	\end{centering}
\end{figure}
...
\begin{figure}[!ht]
	\begin{centering}
		...
		\caption{Spectral energy distribution of 3FGL J0426. \jamie{Replot this make it look nicer}
			\label{fig:J0426SED}}
	\end{centering}
\end{figure}
