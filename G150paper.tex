...
\subsection{What is it?}\label{sec:What}

Why not PWN, radio emission looks more ring/shell like. radio and gamma index are suggestive of SNR, not PWN. GeV matches with radio. no pulsar detected. LAT PWN have broad plateau spectrum, do they extened across the whole energy band? Can I say what edot of the psr would be if I know the LAT flux? 
...
\jamie{naima to do:Add bremss. properly scale the radio flux density. should I be using a power law particle dist function because I have a power law photons spec? increase walker number burn in and runs?Do the energetics make sense? Are there params that I can fix? Kep, B? Any reason to have different n? Liz had doubts about the pp component extending to such high energy, is this really an issue? gtlike finds the best fit index, does the fit value from naima for the particle dist match this? maybe I should fix the particle index based on this? Should the cutoff be the same for e- and p? Do I have to set E0 in naima to 10 GeV for Kep?}
...
\begin{figure}[!ht]
	\begin{centering}
		...
		\caption{ \Gone. Naima SED \jamie{bigger font, get rid of text for each line} }
			\label{fig:naimaSED}
	\end{centering}
\end{figure}
