...
\begin{figure}[!ht]
	\begin{centering}
		...
		\caption{Smoothed background subtracted residual counts map above 1 GeV with 0.1$^\circ$x 0.1$^\circ$ pixels, centered on SNR \Gone. \psrLike{} and the diffuse backgrounds are included in the region model, 3FGL J0425.8+5600 and 3FGL J0423.5+5442 are not. Inset shows the size of the PSF above 1 GeV. \jamie{not sure if I should include the disk fit or not} \jamie{add more description after i remake the map. Add the other 2 3FGL sources? The point of this is to give an sense of what our model thinks the GeV emission from the SNR looks like} \jamie{ redo these removing the extended source?   3FGL removed in a diffetent color? Redo them as pdf/eps. Remove titles, use same cmap as figure 1, bigger bolder font.do I have the TS map for these? Or maybe I can just go into pointlike and give it the right cmap and also add in the 2 other 3FGL sources? something about the emission being clearly extended beyond the extent of the inset PSF}
...
\jamie{what else to include here? Systematics. Bracketing IRFs, still to be done.}
...
\jamie{One potential scenario is that the \gam{}s in this region are produced by a pulsar wind nebula (PWN) generated by the putative puslar of SNR \Gone{}.} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%`%%%%%%
%
%         bib
%
